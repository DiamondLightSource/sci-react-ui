...
export async function init(
  ...
  try {
    ...
    const errorMessage =
      (error as KeycloakError).error ?? JSON.stringify(error);
    addError(
      "Failed to authorise. " +
        errorMessage +
        ". (Are you able to connect to Keycloak?)",
    );
  }
...
export function onAuthRefreshSuccess(
  keycloak: Keycloak,
...
export function tokenRefreshTimer(
  ...
  const minimumSecondsLeftInToken =
      settings.minimumSecondsLeftInToken ??
      DEFAULT_SECONDS_MINIMUM_LEFT_IN_TOKEN,
  ...
  if (timeoutMilliseconds >= 3000) {
    tokenRefreshTimeout = setTimeout(() => {
      keycloak.updateToken(-1).catch((error: KeycloakError) => {
...
export function updateAuth(keycloak: Keycloak | null): Auth {
  const auth: Auth = {
    ...
    refreshToken: (minValidity) =>
      keycloak && keycloak.updateToken(minValidity),
